...
def main():
    ...
    st.sidebar.title("Chapter4")

    sub_menu = ["Spectrum", "FrequencyFilter", "DrawNotchRejectFilter", "RemoveMoire"]
    sub_choice = st.sidebar.selectbox("Select a chapter4 function", sub_menu)
    ...
    elif sub_choice == "FrequencyFilter":
        if "image" in locals():
    ...
    elif sub_choice == "DrawNotchRejectFilter":
        imgout = c4.DrawNotchRejectFilter()
        st.image(imgout, use_column_width=True)

    elif sub_choice == "RemoveMoire":
        if "image" in locals():
